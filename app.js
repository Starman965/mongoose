...
function loadMostRecentWin() {
    ...
    get(ref(database, 'gameSessions')).then((snapshot) => {
        let mostRecentWin = null;
        
        snapshot.forEach((childSnapshot) => {
            const session = childSnapshot.val();
            Object.entries(session.matches || {}).forEach(([matchId, match]) => {
                if (match.placement === 1) {
                    // Check if this match is more recent than the current most recent win
                    if (!mostRecentWin || new Date(session.date) > new Date(mostRecentWin.date)) {
                        mostRecentWin = { ...match, date: session.date };
                    }
                }
            });
        });
        ...
        if (mostRecentWin) {
            recentWinSection.innerHTML = `
                <p><strong>Date:</strong> ${formatDate(mostRecentWin.date)}</p>
                <p><strong>Game Type:</strong> ${mostRecentWin.gameType}</p>
                <p><strong>Game Mode:</strong> ${mostRecentWin.gameMode}</p>
                <p><strong>Map:</strong> ${mostRecentWin.map}</p>
                <p><strong>Total Kills:</strong> ${mostRecentWin.totalKills || 0}</p>
                <p><strong>Placement:</strong> 1st Place</p>
                ${mostRecentWin.highlightURL ? `<p><strong>Highlight Video:</strong> <a href="${mostRecentWin.highlightURL}" target="_blank">Watch</a></p>` : ''}
            `;
        } else {
            recentWinSection.innerHTML = '<p>No recent win found.</p>';
        }
    }).catch((error) => {
        console.error("Error loading most recent win:", error);
        recentWinSection.innerHTML = '<p>Error loading most recent win.</p>';
    });
}
function loadMostRecentAchievement() {
    const recentAchievementSection = document.getElementById('recentAchievementSection');
    ...
    get(ref(database, 'achievements')).then((snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
            ...
            if (achievement.completedOn) {
                // Check if this achievement is more recent than the current most recent one
                if (!mostRecentAchievement || new Date(achievement.completedOn) > new Date(mostRecentAchievement.completedOn)) {
        ...
        if (mostRecentAchievement) {
            recentAchievementSection.innerHTML = `
                <p><strong>Title:</strong> ${mostRecentAchievement.title}</p>
                <p><strong>Description:</strong> ${mostRecentAchievement.description}</p>
                <p><strong>Progress:</strong> ${mostRecentAchievement.progress || 'Completed'}</p>
                <p><strong>Completed On:</strong> ${formatDate(mostRecentAchievement.completedOn)}</p>
                ${mostRecentAchievement.award ? `<p><strong>Award:</strong> ${mostRecentAchievement.award}</p>` : ''}
            `;
        } else {
            recentAchievementSection.innerHTML = '<p>No recent achievement found.</p>';
        }
    }).catch((error) => {
        console.error("Error loading most recent achievement:", error);
        recentAchievementSection.innerHTML = '<p>Error loading most recent achievement.</p>';
    });
}
